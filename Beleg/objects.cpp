...
static void createWireCube()
{
	...
	glGenVertexArrays(1, &VertexArrayIDWireCube); // VertexArrayBuffer Objekt, notiert sich alles was mit Buffern etc. gemacht wird
	...
	glBindVertexArray(0); // VertexArrayObject hört auf sich alles zu "notieren"
}
...
void drawWireCube()
{
	...
	glBindVertexArray(VertexArrayIDWireCube);
	glDrawArrays(GL_LINES, 0, 24); // 24 Punkte -> 12 Linien
}
...
static void createCube()
{
	...
	// gibt id / Pointer auf einen Buffer -> Adresse steht dann in vertexBuffer
	glGenBuffers(1, &vertexbuffer); 
	// VertexBuffer wird aktiviert / gebindet
	glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer); 
	// Daten von RAM in RAM der GPU schreiben ( CPU -> GPU )
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_vertex_buffer_data) /* Größe des Buffer */, g_vertex_buffer_data /* Daten des Buffers */, GL_STATIC_DRAW /* Daten ändern sich nicht */); 
	...
	// Erzeugt 2. Buffer für Farbwerte
	glGenBuffers(1, &colorbuffer);
	// ColorBuffer aktivieren
	glBindBuffer(GL_ARRAY_BUFFER, colorbuffer);
	// ColorBuffer zur GPU schicken
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_color_buffer_data), g_color_buffer_data, GL_STATIC_DRAW);

	// 1. Attribut definieren, VertexPosition
	glEnableVertexAttribArray(0); // Kein Disable ausführen !
	// VertexBuffer in Attribut packen
	glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
	glVertexAttribPointer(
			0,                  // attribute. No particular reason for 0, but must match the layout in the shader.
			3,                  // size
			GL_FLOAT,           // type
			GL_FALSE,           // normalized?
			0,                  // stride, Daten sind lückenlos
			(void*)0            // array buffer offset, beginne an 1. Steller des Buffers
	);
